...
if ( !$ret ) {
	trigger_error('terminate_instances - ~/samples/settings.php not found.', E_USER_ERROR);
}

if ( !isset($params) ) {
	trigger_error('terminate_instances - no variable $params found in settings.php', E_USER_ERROR);
}

$api = new NiftyCloud($params);

$result = $api->terminate_instances($params['instance_id']);
if ( $api->isError() ) {
	echo "error\n";
} else {
